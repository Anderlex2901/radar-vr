...
// ── ORIENTACIÓN POR CUATERNIONES ───────────────────────
let sensor = null;
let calibrado = false;
let baseQuat = null;       // cuaternión de referencia (punto quieto)
let pitchActual = 0;       // eje Y (arriba/abajo)
let yawActual   = 0;       // eje X (izquierda/derecha)
...
function setup() {
  ...
  // Inicializar AbsoluteOrientationSensor (fusión de sensores como Google Maps)
  if (typeof AbsoluteOrientationSensor !== 'undefined') {
    try {
      ...
      sensor.addEventListener('reading', () => {
        ...
        if (baseQuat) {
          // Calcular la rotación relativa: q_rel = q_base_inv * q_actual
          let rel = quatMultiply(quatInverse(baseQuat), q);

          // Extraer pitch (arriba/abajo) y yaw (izquierda/derecha) del cuaternión relativo
          // Pitch: rotación en X
          let sinPitch = 2.0 * (rel[3] * rel[0] + rel[1] * rel[2]);
          let cosPitch = 1.0 - 2.0 * (rel[0] * rel[0] + rel[1] * rel[1]);
          pitchActual = Math.atan2(sinPitch, cosPitch) * (180 / Math.PI);

          // Yaw: rotación en Z
          let sinYaw = 2.0 * (rel[3] * rel[2] + rel[0] * rel[1]);
          let cosYaw = 1.0 - 2.0 * (rel[1] * rel[1] + rel[2] * rel[2]);
          yawActual = Math.atan2(sinYaw, cosYaw) * (180 / Math.PI);
        }
      });
      ...
      console.warn('AbsoluteOrientationSensor no disponible:', e);
    }
  }
}
...
function quatInverse(q) {
  // Para cuaternión unitario, el inverso es el conjugado
...
function manejarControlesMix() {
  ...
  if (calibrado) {
    // pitchActual y yawActual ya son relativos al punto de calibración
    let dx =  yawActual   * 0.25;
    let dy =  pitchActual * 0.25;

    if (abs(yawActual) > 0.4 || abs(pitchActual) > 0.4) {
      p.pos.x += dx * 1.0;
      p.pos.y += dy * 1.0;
      seEstaMoviendo = true;
    }
...
function touchStarted() {
  ...
  // Guardar cuaternión actual como punto de referencia (punto quieto)
