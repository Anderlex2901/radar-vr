...
// ── SENSOR DE MOVIMIENTO (gyroscopio nativo) ───────────

let velX = 0;

let velY = 0;

const sensibilidadX = 0.05;

const sensibilidadY = 0.05;
...
function setup() {
  ...
  for (let i = 0; i < numParticulas; i++) {
    ...
    particulas.push({
      ...
      pos: createVector(0, 0), // La esfera arranca siempre en el centro
...
// Captura nativa del giroscopio, igual que en el código de la cruceta

function capturarMovimiento(event) {

  if (event.rotationRate) {

    let rX = event.rotationRate.alpha || 0;

    let rY = event.rotationRate.beta  || 0;

    velX = (Math.abs(rX) > 0.8) ? rX : 0;

    velY = (Math.abs(rY) > 0.8) ? rY : 0;
...
function manejarControlesMix() {
  ...
  // Movimiento por giroscopio nativo (igual que la cruceta)

  if (permisoConcedido) {

    p.pos.x -= velX * sensibilidadX;

    p.pos.y += velY * sensibilidadY;
    ...
    if (abs(velX) > 0.8 || abs(velY) > 0.8) seEstaMoviendo = true;
...
function touchStarted() {
  ...
  // Activar sensor al tocar la pantalla, sin cartel ni botón
  if (!permisoConcedido) {
