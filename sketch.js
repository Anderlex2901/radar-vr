...
let velX = 0;
let velY = 0;
let sensibilidadX = 0.015; // Calibración fina del prototipo
let sensibilidadY = 0.015;
...
// LÓGICA DE SENSORES INTEGRADA POR VELOCIDAD ANGULAR CRUDA
  ...
  // Soporte auxiliar para desarrollo en Teclado
  ...
  p.pos.x = constrain(p.pos.x, -mitadAncho * 1, mitadAncho * 1);
...
function capturarMovimiento(event) {
  if (event.rotationRate) {
    ...
    // Umbral mínimo (filtro de ruido estático)
    velX = (abs(rX) > 0.8) ? rX : 0;
    velY = (abs(rY) > 0.8) ? rY : 0;
  }
}
...
function touchStarted() {
  ...
  // Activa los sensores silenciosamente con cualquier toque en pantalla
